...
2. Select the target's documented agent representation.
   ...
   - **ChatGPT:** distinguish a Custom GPT from a Workspace Agent. Generate a complete builder brief—name, description, instructions, starters, knowledge, tools, least-privilege action constraints, and test prompts—then ask for confirmation before creating or publishing it in the browser or Workspace Agents app.
